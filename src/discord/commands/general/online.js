...
class OnlineCommand extends Command {
    ...
    async run(msg) {
        if (msg.channel.name === config.settings.inhouseChannel || config.settings.botChannels.indexOf(msg.channel.name) !== -1) {
            const counts = {
                online_no_game: msg.guild.members.filter(member => !member.user.bot && member.presence.status === 'online' && !member.presence.game).size,
            ...
            return msg.say(`Online: ${counts.online_no_game}. In Game: ${counts.online_in_game}. Idle: ${counts.idle}. DND: ${counts.dnd}.`);
        }
    }
}
